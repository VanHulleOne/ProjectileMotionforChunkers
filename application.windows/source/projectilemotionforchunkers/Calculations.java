...
//		this.p = p;
	 ...
	 * calculates the shot taking into account all of the projectile variables plus
 ...
 */
	...
	public double getSlopeSign(Shot shot){
		Shot shot2 = shot(shot.getInitialVelocity(), shot.getAngleRadians()+da/2);
		return ((shot2.getDistance() - shot.getDistance())/(shot2.getAngleRadians() - shot.getAngleRadians()));
	}
	
	public Shot optimizeAngle(double velocity, int j){
		da = Math.abs(da);
		double highAngle = STARTANGLE;
		double guessAngle = STARTANGLE/2;
		double lowAngle = da;
		Shot guess = new Shot();
		...
		do{
			Shot high = shot(velocity, highAngle);
			guess = shot(velocity,guessAngle);
			
	//		double signHigh = getSlopeSign(high);
	//		double signGuess = getSlopeSign(guess);
			double slopeHigh = getSlopeSign(high);
			double slopeGuess = getSlopeSign(guess);
			double signHigh = Math.signum(slopeHigh);
			double signGuess = Math.signum(slopeGuess);
			
			System.out.println("  Slope High: " + slopeHigh + "\tSlope Guess: " + slopeGuess);
			i += 4;
			
			if(signGuess == 0) break;
			if(signHigh == signGuess){
				highAngle = guessAngle;
			}
			else{
				lowAngle = guessAngle;
			}
			guessAngle = (highAngle - lowAngle)/2 + lowAngle;
			System.out.print("\tVi: " + guess.getInitialVelocity());
			System.out.print("\tAngle: " + guess.getAngleDegrees());
			System.out.println("\tDistance: " + guess.getDistance());
//			System.out.println("Low angle: " + lowAngle);
//			System.out.println("Guess angle: " + guessAngle);
//			System.out.println("High angle: " + highAngle);
		}
		while( (highAngle - lowAngle)> (da/1));
		
//		System.out.println("Low angle: " + lowAngle);
		System.out.println("Guess angle: " + guessAngle);
//		System.out.println("High angle: " + highAngle);
		
		System.out.println("Line 237: New Opt: " + i);
		hasPrinted = true;
 ...
 */
	public Shot optimizeAngle(double velocity){
		...
		while(currentShot.compareTo(previousShot) >= 0){
			if(hasPrinted){
//				System.out.print("\tVi: " + currentShot.getInitialVelocity());
//				System.out.print("\tAngle: " + currentShot.getAngleDegrees());
//				System.out.println("\tDistance: " + currentShot.getDistance());
			}
		...
		if(hasPrinted){
			System.out.println("Line 264: OldOpt: " + i);
			System.out.println("Old angle: " + previousShot.getAngleRadians());
			System.out.println(previousShot);
			hasPrinted = false;
		}
		return previousShot; //return the previous shot which went the farthest
	}
